...
export default function SaturationSlider({value, setValue} : {value:number[], setValue: Function}) {
    ...
    if (canvas) {
        ...
        for (let column = 0; column < 50; column+=1) {
            for (let row =0; row < 100; row+=1) {
                const pixel = column *(100 *4) + (row*4)
                const values = hsv2rgb(360, row/100, 1);
  ...
  return (
    <>
        <div className="relative">
            <canvas ref={canvasRef} height="50" width="100" className="absolute top-0 left-0">
            </canvas>
