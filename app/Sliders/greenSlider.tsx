...
export default function GreenSlider({value, setValue} : {value:number[], setValue: Function}) {
  ...
  useEffect(()=> {
    ...
    if (canvas) {
        ...
        ctx.rect(0, 0, 255, 50)
        ctx.fill();
        const imageData = ctx.getImageData(0, 0, 255, 50);
        const data = imageData.data;

        for (let column = 0; column < 50; column+=1) {
            for (let row =0; row < 255; row+=1) {
                const pixel = column *(255 *4) + (row*4)
                data[pixel] = 0;
                data[pixel+1] = 0;
                data[pixel+2] = row;
            }
  ...
  return (
    <>
        <div className="relative">
            <canvas ref={canvasRef} height="50" width="255" className="absolute top-0 left-0">
            </canvas>
