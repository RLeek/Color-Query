    ...
    <main>
      <div className="flex flex-col justify-center place-items-center">
        ...
        <div className="w-10/12 md:w-144 xl:w-160">
          <textarea autoCapitalize="none" onChange={e => setText(e.target.value)} className="w-full mt-12 mb-4 px-2 pt-2 h-10svh"></textarea>
          {error != "" && <div className="pb-4 pl-2 text-red-600">{error}</div>}
          <div className="flex flex-row place-content-between">
            <button onClick={submit} className="bg-white rounded-lg p-2 hover:bg-slate-200">
              Submit Query
            </button>
            <div>
              <button onClick={reset} className="bg-white rounded-lg p-2 mx-4 hover:bg-slate-200">
                Reset Image
              </button>
              <button onClick={clear} className="bg-white rounded-lg p-2 hover:bg-slate-200">
                Clear Image
              </button>
            </div>
          </div>
